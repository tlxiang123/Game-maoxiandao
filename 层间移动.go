...
func 随机方向移动(方向键 int, x, 目标左, 目标右, 限制左, 限制右 int) {
	用跳跃 := 应使用走X() && 可用跳跃移动(方向键, x, 目标左, 目标右, 限制左, 限制右)
	if 用跳跃 {
...
func 应使用走X() bool {
	...
	now := time.Now()
	if 移动方式周期截止.IsZero() || !now.Before(移动方式周期截止) {
		普通走概率百分比 = 普通走最小概率百分比 + 移动随机.Intn(普通走最大概率百分比-普通走最小概率百分比+1)
		移动方式周期截止 = now.Add(移动方式周期)
		输出("移动方式比例重置", "普通走=", 普通走概率百分比, "%", "走+X=", 100-普通走概率百分比, "%")
	}

	return 移动随机.Intn(100) >= 普通走概率百分比
}
