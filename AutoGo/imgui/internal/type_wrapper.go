...
// Supports both signed (int8) and unsigned (uint8) char types for Android compatibility
func WrapString[RET ~int8 | ~uint8](value string) (wrapped *RET, finisher func()) {
...
// Default value of RET is C.char
// Supports both signed (int8) and unsigned (uint8) char types for Android compatibility
func WrapStringList[RET ~int8 | ~uint8](value []string) (wrapped **RET, finisher func()) {
	if len(value) == 0 {
		return nil, func() {}
	}
