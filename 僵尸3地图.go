...
const (
	...
	僵尸3靠近绳子慢走距离  = 8
	僵尸3绳子精确对齐距离  = 3
	僵尸3上绳按键延迟    = 70 * time.Millisecond
	僵尸3绳子对齐短按    = 70 * time.Millisecond
	僵尸3绳子精确对齐短按  = 45 * time.Millisecond
	僵尸3绳子对齐等待    = 120 * time.Millisecond
	僵尸3绳子校正空格次数  = 3
	僵尸3绳子校正最短    = 70 * time.Millisecond
	僵尸3绳子校正最长    = 70 * time.Millisecond
	僵尸3绳子到位稳定等待  = 500 * time.Millisecond
	僵尸3击退反向容差    = 1
	僵尸3三层换层等待    = 1500 * time.Millisecond
	僵尸3三层下绳检测    = 1 * time.Second
	僵尸3三层左下下绳超时  = 4 * time.Second
	僵尸3左侧进三层超时   = 18 * time.Second
	僵尸3卡住检测时间    = 8 * time.Second
	僵尸3卡住报警时间    = time.Minute
	僵尸3卡住坐标容差    = 1
	僵尸3刷怪卡住检测时间  = 3 * time.Second
	僵尸3一层爬绳最短间隔  = 118 * time.Second
	僵尸3一层爬绳最长间隔  = 233 * time.Second
)
...
func 读取僵尸3输出文本() string {
	僵尸3输出锁.Lock()
	defer 僵尸3输出锁.Unlock()
	start := len(僵尸3输出行) - 僵尸3输出显示行数
	if start < 0 {
		start = 0
	}
	return strings.Join(僵尸3输出行[start:], "\n")
}

func 读取僵尸3全部输出文本() string {
	僵尸3输出锁.Lock()
	defer 僵尸3输出锁.Unlock()
...
func 僵尸3移动到绳子正下方(shouldContinue func() bool) (层位置, bool) {
	for shouldContinue() {
		...
		if diff <= 僵尸3靠近绳子慢走距离 {
			duration := 僵尸3绳子对齐短按
			if diff <= 僵尸3绳子精确对齐距离 {
				duration = 僵尸3绳子精确对齐短按
			}
			僵尸3按移动并检测反向击退(位置, 方向键, 僵尸3绳子对齐等待, func() {
				僵尸3按方向键指定时长(方向键, duration)
			})
			continue
		} else if 僵尸3应使用海盗走X(位置.X, 目标X, 方向键, 僵尸3一层刷怪左边, 僵尸3一层右边, false) {
...
func 运行僵尸3循环(runID int64, 启动先三层卖物品 bool) {
	最短分钟, 最长分钟 := 读取僵尸3卖物品分钟范围()
	设置僵尸3层输出("僵尸3开始：1-2层正常刷怪，按%d~%d分钟前往3层卖物品", 最短分钟, 最长分钟)
	启动N键守护(runID)
	卖物品截止 := 新僵尸3卖物品截止()
	一层爬绳截止 := 新僵尸3一层爬绳截止()
	首次一层 := true
	...
	for 脚本仍应运行(runID) {
		...
		if 僵尸3已到卖物品时间(&卖物品截止) {
			if !僵尸3按计时前往三层卖物品后回一层(runID, &卖物品截止) {
				continue
			}
			首次一层 = true
			一层爬绳截止 = 新僵尸3一层爬绳截止()
...
func 新僵尸3卖物品截止() time.Time {
	duration := 随机僵尸3卖物品周期时长()
	最短分钟, 最长分钟 := 读取僵尸3卖物品分钟范围()
	设置僵尸3层输出("下次卖物品：%d分钟后（范围%d~%d分钟）", int(duration.Minutes()), 最短分钟, 最长分钟)
	return time.Now().Add(duration)
}

func 读取僵尸3卖物品分钟范围() (int32, int32) {
	最短分钟 := 僵尸3卖物品最短分钟
	最长分钟 := 僵尸3卖物品最长分钟
	if 最短分钟 < 1 {
		最短分钟 = 1
...
func 设置僵尸3卖物品分钟范围(最短分钟, 最长分钟 int32) {
	if 最短分钟 < 1 {
		最短分钟 = 1
	}
	if 最长分钟 < 1 {
...
func 随机僵尸3卖物品周期时长() time.Duration {
	最短分钟, 最长分钟 := 读取僵尸3卖物品分钟范围()
	spanMinutes := int64(最长分钟 - 最短分钟)
	if spanMinutes <= 0 {
		return time.Duration(最短分钟) * time.Minute
	}
	return time.Duration(最短分钟)*time.Minute + time.Duration(移动随机.Int63n(spanMinutes+1))*time.Minute
}

func 僵尸3已到卖物品时间(卖物品截止 *time.Time) bool {
	return 卖物品截止 != nil && !time.Now().Before(*卖物品截止)
}

func 僵尸3按计时前往三层卖物品后回一层(runID int64, 卖物品截止 *time.Time) bool {
	if !僵尸3已到卖物品时间(卖物品截止) {
		return true
	}
	设置僵尸3层输出("到卖物品时间：先前往3层再卖物品")
	if !僵尸3确保到三层(runID) {
		设置僵尸3层输出("前往3层卖物品中断，稍后重试")
		return false
	}
	return 僵尸3三层按计时处理后回一层(runID, 卖物品截止)
}

func 僵尸3确保到三层(runID int64) bool {
	for 脚本仍应运行(runID) {
		位置, ok := 僵尸3当前层位置()
		if !ok {
			time.Sleep(僵尸3未知位置等待)
		...
		switch 位置.层 {
		case 3:
			设置僵尸3层输出("卖物品位置确认：已在3层")
			return true
		case 2:
			设置僵尸3层输出("卖物品路线：2层先下到1层")
			僵尸3二层下到一层直到成功(runID)
		case 1:
			设置僵尸3层输出("卖物品路线：1层前往3层")
			僵尸3一层到三层直到成功(runID)
		default:
			设置僵尸3层输出("卖物品路线失败：当前层=%d", 位置.层)
			return false
		}
		time.Sleep(僵尸3移动间隔)
	}
...
func 僵尸3三层执行卖物品(runID int64) bool {
	位置, ok := 僵尸3当前层位置()
	if !ok {
		设置僵尸3层输出("卖物品取消：无法确认当前在3层")
		return false
	}
	if 位置.层 != 3 {
		设置僵尸3层输出("卖物品取消：当前在%d层，只允许3层卖物品", 位置.层)
		return false
	}
	设置僵尸3层输出("3层开始卖物品：使用海盗同一套买卖物品流程")
	if !执行僵尸3完整买卖物品流程(func() bool { return 脚本仍应运行(runID) }) {
		设置僵尸3层输出("3层卖物品中断，稍后重试")
		return false
	}
	return true
}
...
func 僵尸3随机尝试一层到三层方式(shouldContinue func() bool) bool {
	if 移动随机.Intn(2) == 0 {
		...
		设置僵尸3层输出("右侧触发未成功，改用左侧出边界")
		return 僵尸3一层到三层左侧出边界(shouldContinue)
	}
	if 僵尸3一层到三层左侧出边界(shouldContinue) {
		return true
	}
	设置僵尸3层输出("左侧出边界未成功，改用右侧触发")
	return 僵尸3一层到三层右侧触发(shouldContinue)
}

func 僵尸3一层到三层右侧触发(shouldContinue func() bool) bool {
	设置僵尸3层输出("1层到3层方式：到%d左边后右+X", 僵尸3去三层触发X)
	if !僵尸3移动到X范围直到(shouldContinue, 1, 僵尸3去三层准备左, 僵尸3去三层准备右, 僵尸3一层刷怪左边, 僵尸3去三层触发X) {
		return false
	}
	位置, ok := 僵尸3当前层位置()
	if ok && 位置.层 == 3 {
...
func 僵尸3一层到三层左侧出边界(shouldContinue func() bool) bool {
	设置僵尸3层输出("1层到3层方式：从1层左侧走到x=%d出边界", 僵尸3左侧进三层目标X)
	deadline := time.Now().Add(僵尸3左侧进三层超时)
	for shouldContinue() && time.Now().Before(deadline) {
		位置, ok := 僵尸3当前层位置()
		if ok {
			if 位置.层 == 3 {
				设置僵尸3层输出("1层到3层成功：左侧出边界")
				return true
			}
			if 位置.层 != 1 {
				return false
			}
			按组合键不空格(motion.KEYCODE_DPAD_LEFT, motion.KEYCODE_X, 僵尸3左侧进三层冲刺毫秒)
		} else {
			按组合键不空格(motion.KEYCODE_DPAD_LEFT, motion.KEYCODE_X, 僵尸3左侧进三层冲刺毫秒)
		}
		按空格群攻()
...
func 僵尸3三层执行下绳到一层(起始位置 层位置, shouldContinue func() bool) bool {
	displayID := 当前显示ID()
	设置僵尸3层输出("3层下1层：先按住左+下，向左越过x=%d后改按右+下校正", 僵尸3三层下绳左界)
	当前方向 := motion.KEYCODE_DPAD_LEFT
	motion.KeyActionDown(当前方向, displayID)
	...
	for shouldContinue() && time.Now().Before(deadline) {
		...
		目标方向 := 当前方向
		if 位置.X < 僵尸3三层下绳左界 {
			目标方向 = motion.KEYCODE_DPAD_RIGHT
		} else if 位置.X > 僵尸3三层下绳X+僵尸3三层下绳容差 {
			目标方向 = motion.KEYCODE_DPAD_LEFT
		}
		if 目标方向 != 当前方向 {
			设置僵尸3层输出("3层下1层：走过头 x=%d，改按%s+下", 位置.X, 键名(目标方向))
			motion.KeyActionUp(当前方向, displayID)
			motion.KeyActionDown(目标方向, displayID)
			当前方向 = 目标方向
		}
		已往下移动 := 位置.层 < 起始位置.层 || 位置.Y >= 起始位置.Y+僵尸3爬绳上升Y阈值
		if 已往下移动 {
			设置僵尸3层输出("3层下1层：已下降 x=%d y=%d，%s+Z脱离绳子", 位置.X, 位置.Y, 键名(当前方向))
			motion.KeyActionUp(motion.KEYCODE_DPAD_DOWN, displayID)
			motion.KeyActionUp(当前方向, displayID)
			按组合键不空格(当前方向, motion.KEYCODE_Z, 方向键按下毫秒)
			return 僵尸3等待到层(1, 僵尸3三层换层等待, shouldContinue)
		}
		time.Sleep(僵尸3爬绳检测间隔)
	}
	设置僵尸3层输出("3层下1层：方向+下超时未检测到下降，重试")
	return false
}

func 僵尸3一层走到X(runID int64, 目标X int) bool {
	return 僵尸3层内走到X(runID, 1, 目标X, 僵尸3一层刷怪左边, 僵尸3一层右边, false)
}

func 僵尸3二层随机巡逻(runID int64) bool {
	巡逻次数 := 僵尸3二层巡逻最少次数 + rand.Intn(僵尸3二层巡逻最多次数-僵尸3二层巡逻最少次数+1)
	设置僵尸3层输出("2层本次随机巡逻%d轮", 巡逻次数)
	for i := 1; i <= 巡逻次数 && 脚本仍应运行(runID); i++ {
		设置僵尸3层输出("2层巡逻%d/%d：左到右", i, 巡逻次数)
...
func 僵尸3巡逻边缘已到位(x, 目标X, 左边, 右边 int) bool {
	if 目标X == 左边 {
		return x <= 左边+跳跃移动像素
	}
	if 目标X == 右边 {
...
func (监控 *僵尸3刷怪卡住监控) 检查并执行打怪(场景 string, 位置 层位置) bool {
	now := time.Now()
	if !监控.有上次坐标 || absInt(位置.X-监控.上次X) > 僵尸3卡住坐标容差 || absInt(位置.Y-监控.上次Y) > 僵尸3卡住坐标容差 {
		监控.有上次坐标 = true
		监控.上次X = 位置.X
		监控.上次Y = 位置.Y
		监控.固定开始 = now
	...
	if now.Sub(监控.固定开始) < 僵尸3刷怪卡住检测时间 {
		return false
	}
	设置僵尸3层输出("%s：坐标%d秒未动 x=%d y=%d，执行打怪脱困", 场景, int(僵尸3刷怪卡住检测时间.Seconds()), 位置.X, 位置.Y)
	打怪动作(位置)
	time.Sleep(近怪攻击间隔)
	监控.有上次坐标 = true
	监控.上次X = 位置.X
...
func 僵尸3二层下到一层直到成功(runID int64) bool {
	...
	for 脚本仍应运行(runID) {
		...
		if 位置.X < 僵尸3二层可走X左 {
			...
			switch 僵尸3执行下绳动作(位置.层, 脱离方向, func() bool { return 脚本仍应运行(runID) }) {
			case 僵尸3下绳成功:
				设置僵尸3层输出("已回到1层")
				return true
			case 僵尸3下绳脱困成功:
				if 脱困位置, ok := 僵尸3当前层位置(); ok && 脱困位置.层 == 1 {
					设置僵尸3层输出("2层下绳脱困成功：回到1层后立即打怪，再继续正常刷怪")
					打怪动作(脱困位置)
					time.Sleep(近怪攻击间隔)
				} else {
					设置僵尸3层输出("2层下绳脱困成功：未稳定识别到1层，回主循环继续处理")
				}
				return false
			}
...
func 僵尸3执行下绳动作(起始层 int, 脱离方向 int, shouldContinue func() bool) 僵尸3下绳结果 {
	displayID := 当前显示ID()
